...
def test_successful_filling_students_registration_form():
    ...
    s('#userEmail-wrapper').should(have.exact_text('Email'))
    ...
    s('#dateOfBirth-label').should(have.exact_text('Date of Birth'))
    ...
    hobbies[3].should(have.exact_text('Sports')).click()
    hobbies[4].should(have.exact_text('Reading')).click()
    hobbies[5].should(have.exact_text('Music')).click()
    s('#uploadPicture').send_keys(os.path.abspath(file_path))
    s('#currentAddress-wrapper').should(have.exact_text('Current Address'))
